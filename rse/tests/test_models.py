...
class SalaryCalcultionTests(TestCase):
    ...
    def setUp(self):
        ...
        sb11_2017 = SalaryBand(grade=1, grade_point=1, salary=(1000),
                               year=y2017, increments=True)
        sb11_2017.save()
        sb12_2017 = SalaryBand(grade=1, grade_point=2, salary=(2000),
                               year=y2017, increments=True)
        sb12_2017.save()
        sb13_2017 = SalaryBand(grade=1, grade_point=3, salary=(3000),
                               year=y2017, increments=True)
        sb13_2017.save()
        # Create non incremental range
        sb14_2017 = SalaryBand(grade=1, grade_point=4, salary=(4000),
                               year=y2017, increments=False)
        sb14_2017.save()
        sb15_2017 = SalaryBand(grade=1, grade_point=5, salary=(5000),
                               year=y2017, increments=False)
        sb15_2017.save()

        # Create an incremental range for 2018 year
        sb11_2018 = SalaryBand(grade=1, grade_point=1, salary=(1001),
                               year=y2018, increments=True)
        sb11_2018.save()
        sb12_2018 = SalaryBand(grade=1, grade_point=2, salary=(2001),
                               year=y2018, increments=True)
        sb12_2018.save()
        sb13_2018 = SalaryBand(grade=1, grade_point=3, salary=(3001),
                               year=y2018, increments=True)
        sb13_2018.save()
        # Create non incremental range
        sb14_2018 = SalaryBand(grade=1, grade_point=4, salary=(4001),
                               year=y2018, increments=False)
        sb14_2018.save()
        sb15_2018 = SalaryBand(grade=1, grade_point=5, salary=(5001),
                               year=y2018, increments=False)
        sb15_2018.save()

        # Create an incremental range for 2019 year
        sb11_2019 = SalaryBand(grade=1, grade_point=1, salary=(1002),
                               year=y2019, increments=True)
        sb11_2019.save()
        sb12_2019 = SalaryBand(grade=1, grade_point=2, salary=(2002),
                               year=y2019, increments=True)
        sb12_2019.save()
        sb13_2019 = SalaryBand(grade=1, grade_point=3, salary=(3002),
                               year=y2019, increments=True)
        sb13_2019.save()
        # Create non incremental range
        sb14_2019 = SalaryBand(grade=1, grade_point=4, salary=(4002),
                               year=y2019, increments=False)
        sb14_2019.save()
        sb15_2019 = SalaryBand(grade=1, grade_point=5, salary=(5002),
                               year=y2019, increments=False)
        sb15_2019.save()
    ...
    def test_salary_year_span(self):
        """
        Just a bunch of simple tests to check the financial used to see 
        if dates span the financial year.
        """
        # Obvious true
        self.assertEqual(SalaryBand.spansFinancialYear(date(2017, 1, 1),
                                                       date(2018, 1, 1)), True)
        # True by a long way
        self.assertEqual(SalaryBand.spansFinancialYear(date(2017, 1, 1),
                                                       date(2020, 8, 1)), True)
    ...
    def test_salary_increment_final_year(self):
        """
        Test will check the incremented salary band for a salary band with no
        data for the next year.

        The correct behaviour is that the salary band should be estimated by
        providing the next increment from the current year
        i.e. G1.1 from 2019 should be incremented to G1.2 2019
        """
        sb11 = SalaryBand.objects.filter(grade=1, grade_point=1,
                                         year__year=2019)[0]
        ...
        # Salary grade point should be based of 2019 data 
        # as no 2020 data exists
        self.assertEqual(sb12.salary, 2002)

    def test_salary_increment_first_year(self):
        """
        Test will check the incremented salary band for a salary band with data
        available for the next financial year.

        The correct behaviour is that the salary band should be incremented
        using the next financial years data
    ...
    def test_salary_increment(self):
        """
        Test will check the that a non incrementing salary band remains on the
        same grade point but uses the next financial years data.

        The correct behaviour is that the salary band should be not be
        incremented but the next years financial data should be used
        i.e. G1.4 from 2017 should be incremented to G1.4 2018.
        """
        sb14_2017 = SalaryBand.objects.filter(grade=1, grade_point=4,
                                              year__year=2017)[0]
    ...
    def test_salary_projection(self):
        # NB first grade change is for 2017
        sgc = SalaryGradeChange.objects.filter(rse=self.rse)[0]
        ...
        self.assertEqual(sb.salary, 1000)
        # Check salary for rse at first date in next financial year (should be
        # normal increment using 2018 data)
        sb = sgc.salaryBandAtFutureDate(date(2018, 8, 1))
        self.assertEqual(sb.salary, 2001)
        # Check salary for rse at date in next financial year (should be normal
        # increment using 2018 data)
        sb = sgc.salaryBandAtFutureDate(date(2018, 12, 12))
        self.assertEqual(sb.salary, 2001)
        # Check salary for rse at a date two years in future (should be
        # retrieved from salary band change to g1.5)
        sb = sgc.salaryBandAtFutureDate(date(2019, 10, 10))
        self.assertEqual(sb.salary, 5002)
        # Check salary for rse at a date many years in future (should be
        # retrieved from salary band change to g1.5) which does not increment
        sb = sgc.salaryBandAtFutureDate(date(2050, 10, 10))
        self.assertEqual(sb.salary, 5002)

    def test_query_salary_band(self):
        # Should error as no salary band data changes exist before 1st August
        # 2017
        # self.assertEqual(self.rse.futureSalaryBand(date(2017, 1, 1)).salary,
        #                  1000)
        # Should return a salary band of g1.1 2017
        self.assertEqual(self.rse.futureSalaryBand(date(2017, 12, 12)).salary,
                         1000)
        # Should return a salary band of g1.1 2017
        self.assertEqual(self.rse.futureSalaryBand(date(2018, 1, 1)).salary,
                         1000)
        # Should return a salary band of g1.2 2018
        self.assertEqual(self.rse.futureSalaryBand(date(2018, 9, 1)).salary,
                         2001)
    ...
    def test_allocation_costs(self):
        ...
        # Test cost of 50% allocation for full 2017 and 2018 year (expects
        # 1000*.5 + 2001*0.5)
