...
from .models import RSE, Project, RSEAllocation, User
...
def rse_view(request, rse_username):
    ...
    # NB lambda function returns a if b is None or f(a,b) if b is not none
    f_bnone = lambda f, a, b: a if b is None else f(a, b)
    starts = [[f_bnone(max, item.start, from_date), item.percentage, item]
              for item in allocations]
    ends = [[f_bnone(min, item.end, until_date), -item.percentage, item]
            for item in allocations]
    ...
    staff_recovered = sum([a.staffCost(from_date, until_date)
    ...
    view_dict['filter_date'] = from_date.strftime("%d/%m/%Y") + ' - ' + until_date.strftime("%d/%m/%Y")
