...
class FinancialYear(models.Model):
    # NB must relate to a financial year
    year = models.IntegerField(primary_key=True, default=2018)
    ...
    def salaryBandAfterIncrement(self):
        ...
        if sbs:  # should be unique
            ...
            if not self.increments:
                return self
            # If there is an increment then
            # get next salary band from current year
            sbs = SalaryBand.objects.filter(grade=g, grade_point=gp,
                                            year=self.year)
            if sbs:  # should be unique
                return sbs[0]
            else:
...
class RSE(models.Model):
    ...
    # Gets the last salary grade change before the specified date
    # (i.e. the last appropriate grade change) or now
    def lastSalaryGradeChange(self, _date=datetime.now()):
        sgcs = SalaryGradeChange.objects.filter(rse=self).order_by('-salary_band__year')
        for sgc in sgcs:
            if sgc.salary_band.startDate() <= _date:
                return sgc
    ...
    def futureSalaryBand(self, date):
        ...
        # this may be based on real grade changes in the future or estimated
        # from current financial year increments
    ...
    def staffCost(self, start, end, percentage=100.0):
        # Add one to end as end day is inclusive or cost calculation
        # (e.g.  start date from 9:00 end date until 5pm)
        end = end + timedelta(days=1)

        # Calculate cost per year by splitting into financial year periods to
        # obtain correct salary info
        cost = 0
        s = start
        while SalaryBand.spansFinancialYear(s, end):
            # Salary band at date s
            sb = self.futureSalaryBand(s)
            # Financial year end for date s
            # (for cost calculations this needs to include this day)

            ye = sb.endDate() + timedelta(days=1)
            d = ye - s  # Duration in days
            # Increment by salary cost for this period (for allocation %age)
            cost += self.salaryCost(d.days, sb.salary, percentage)
            # Increment to next financial year
            s = ye

        # Final (partial) year calculation
        d = end - s  # Duration in days
        # Get latest salary for this period
        sb = self.futureSalaryBand(s)
        # increment by salary cost for this period
        cost += self.salaryCost(d.days, sb.salary, percentage)
...
class SalaryGradeChange(models.Model):
    ...
    def salaryBandAtFutureDate(self, date):
        ...
        if not SalaryBand.spansFinancialYear(self.salary_band.startDate(),
                                             date):
    ...
    def __str__(self):
        return (self.rse.user.first_name + ' ' + self.rse.user.last_name +
                ': Grade ' + str(self.salary_band.grade) + '.' +
                str(self.salary_band.grade_point) +
...
class RSEAllocation(models.Model):
    ...
    def __str__(self):
        return f"{self.rse.user} on {self.project} at {self.percentage}%"
