...
By default the RSEAdmin tool will use the development settings (located in [RSEAdmin/settings/dev.py](`RSEAdmin/settings/dev.py`)). For production there are various development settings which are not ideal (i.e. a public secret key, debug mode, choice of database). A settings file [RSEAdmin/settings/pythonanywhere.py](`RSEAdmin/settings/pythonanywhere.py`) is provided for the [http://www.pythonanaywhere.com](Python Anywhere) who provide a free tier of hosting for Django sites.
...
A [https://github.com/RSE-Sheffield/rseadmin-ansible](separate repo is available) to provide instructions for deploying on your own virtual machines.
